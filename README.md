...
- 如本地存在合适的 LaTeX 编译工具链，则优先本地编译并产出 PDF
- 如本地没有合适工具链，则仍生成可直接上传到 Overleaf 编译的项目结构
...
6. 如果本地有可用 LaTeX 工具链，让代理先在本地编译并检查 PDF；如果没有，再将整个生成后的项目文件夹上传到 Overleaf 编译。
...
- 若当前环境具备合适的 LaTeX 编译工具链，代理应尽量额外产出本地编译得到的 PDF
...
- 如果当前环境具备合适的 LaTeX 编译工具链，代理应优先本地实际编译并修正错误；否则，生成的 LaTeX 工程应避免绝对路径、本地专属字体、本地脚本依赖和需要 shell-escape 的方案，以保证 Overleaf 可直接编译。
